...
`/Users/paulyhedral/Projects/Code/FullTime/worktrees/<repo-slug>-<branch-slug>`. All
...
If this repo is itself a submodule of a parent meta-repo (`Apps`, `Libs/openligadb`, or the
top-level `FullTime`), walk up: in the parent's main working tree, `git add <submodule-path>`
and commit the updated submodule reference, then push - repeat at each level up to the
umbrella `FullTime` repo if the change touched a nested submodule (see `docs/git-submodules.md`).

### 11. Remove the feature worktree

```
git worktree remove /Users/paulyhedral/Projects/Code/FullTime/worktrees/<repo-slug>-<branch-slug>
...
- Nested submodule repos need the submodule-pointer commit propagated at every level between
  where the code lives and the umbrella `FullTime` repo, not just one level up.
