...
OpenSpec changes live in the umbrella `FullTime` repo or in a submodule (`Apps`,
`Libs/openligadb`, or their nested repos - see
...
```
git worktree add /Users/paulyhedral/Projects/Code/FullTime/worktrees/<repo-slug>-<branch-slug> -b <branch-name> origin/develop
```

- `<repo-slug>`: the repo path with `/` replaced by `-` (e.g. `fulltime-libs-openligadb`).
